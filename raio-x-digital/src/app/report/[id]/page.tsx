...
import { DownloadPdfButton } from "@/features/xray-auditor/components/DownloadPdfButton";
...
export default async function ReportPage(props: { params: Promise<{ id: string }> }) {
    ...
    return (
        <main className="min-h-screen bg-[#020617] text-slate-50 font-sans p-4 sm:p-8 flex flex-col items-center">
            ...
            <div id="report-content" className="w-full max-w-4xl pt-8 pb-16 relative z-10 bg-[#020617] rounded-3xl">
                <header className="mb-10 text-center flex flex-col items-center">
                    <div className="w-20 h-20 mb-6 bg-black/40 rounded-2xl p-2 border border-white/10 shadow-[0_0_20px_rgba(139,92,246,0.3)]">
                        <img
                            src="/logo-gw.png"
                            alt="Grooway"
                    ...
                    <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight mb-2 text-transparent bg-clip-text bg-gradient-to-r from-indigo-300 to-white">
                        Auditoria de Posicionamento Digital
                    </h1>
                    <p className="text-slate-400 text-lg mb-8">
                        Dossiê Exclusivo de <span className="font-bold text-white">{reportData.target_url}</span>
                    </p>

                    <DownloadPdfButton targetId="report-content" filename={`Dossie-OURo-${reportData.target_url.replace(/[^a-zA-Z0-9]/g, '-')}`} />
                </header>
                ...
                <div className="space-y-6">
                    {reportData.skills_results.map((skill, index) => (
                        <div key={index} className="bg-black/30 backdrop-blur-sm p-6 rounded-2xl border border-white/10 hover:border-white/20 transition-all break-inside-avoid">
                ...
                <div className="mt-16 text-center border-t border-slate-800 pt-8 pb-10 break-inside-avoid">
