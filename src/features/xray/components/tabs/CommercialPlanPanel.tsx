                        ...
                        {commercialPlan.servicos_recomendados.map((srv: any, idx: number) => (
                            <div key={idx} className="bg-black/30 backdrop-blur-sm border border-indigo-500/20 rounded-2xl p-5 flex flex-col hover:bg-black/50 transition-all shadow-lg group">
                                ...
                                <div className="space-y-4">
                                    <p className="text-sm text-slate-300 leading-relaxed bg-white/5 p-3 rounded-xl">{srv.por_que_vender}</p>
                                    <p className="text-sm text-emerald-100/80 font-bold flex items-center gap-1"><ArrowRight className="w-3" /> {srv.impacto_esperado}</p>
                                </div>
                            </div>
                        ))}
                    </div>
